/**
 * sw-roleplay-relay — Claude API への中継。
 *
 * 目的は2つだけ。
 *   1. APIキーをブラウザに置かずに済ませる（キーはWorkerのsecretに置く）
 *   2. 課金が持ち主に来るので、使いすぎを仕組みで止める
 *
 * 中継するのは POST /v1/messages のみ。ストリーミングはそのまま素通しし、
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (!Number.isFinite(body.max_tokens) || body.max_tokens > cap) body.max_tokens = cap;
    ...
    if (ipCount >= int(env.DAILY_REQUESTS_PER_IP, 60)) {
      return err(429, "本日の利用回数の上限に達しました。明日また試してください。", cors);
    }
    if (tokUsed >= int(env.DAILY_OUTPUT_TOKENS, 300000)) {
...
/** SSE / JSON のどちらでも usage.output_tokens を拾って日次カウンタに足す。 */
async function countOutputTokens(stream, kv, key) {
  try {
    const text = await new Response(stream).text();
    let out = 0;
    // ストリーミング: message_delta の usage が最終値
...
function corsHeaders(origin, env) {
  ...
  return {
    ...
    "Access-Control-Allow-Headers": "content-type, x-access-code, anthropic-version, anthropic-beta, x-api-key, anthropic-dangerous-direct-browser-access",
...
function err(status, message, cors) {
  return new Response(JSON.stringify({ type: "error", error: { type: "relay_error", message } }), {
    status,
